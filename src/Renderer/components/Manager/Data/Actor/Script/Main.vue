<template>
  <file-generator-component
    ...
    :description="[
      '액터의 내용을 프로그래밍합니다.',
      '하지만 액터 파일에서 모든 내용을 프로그래밍하기는 불편하고, 관리하기 힘듭니다.<br>스크립트는 프로그래밍 코드를 쪼개서 관리할 수 있도록 도와줍니다.',
      '이곳에서 추가한 스크립트는 자동으로 해당 액터에 등록됩니다.'
    ]"
...
export default class ScriptMainComponent extends Vue {
  ...
  private async add(scriptFilePath: string): Promise<void> {
    ...
    const scriptAdd: Engine.GameProject.AddActorScriptSuccess|Engine.GameProject.AddActorScriptFail = await ipcRenderer.invoke('add-actor-script', this.projectDirectory, this.filePath, scriptFilePath)
  ...
  private goBack(): void {
    this.$router.replace('/manager/actor').catch(() => null)
  }
  ...
  private async setCwd(): Promise<void> {
    const pathGet: Engine.GameProject.GetStoragePathSuccess|Engine.GameProject.GetStoragePathFail = await ipcRenderer.invoke('get-storage-path', this.projectDirectory, this.storageKey, PROJECT_SRC_STORAGE_ACTOR_SCRIPT_DIRECTORY_NAME, '')
    if (!pathGet.success) {
